...
physics::Atom loadAtom(const std::string& filename) {
    ...
    const auto photo = internal::parse_sigma(cs.at("photo"));
    const auto compton = internal::parse_sigma(cs.at("compton"));
    const auto pair = internal::parse_sigma(cs.at("pair"));

    return physics::Atom(symbol, Z, A, photo, compton, pair);
}
